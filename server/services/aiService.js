...
const hasOpenAIKey = () => {
  ...
  return key && key !== 'your_openai_api_key_here' && key !== 'sk-your_openai_api_key_here' && !key.includes('your_openai_api_key');
};
...
export const generateAIInsights = async ({ transactions, budget }) => {
  ...
  try {
    ...
    // Clean potential markdown fencing if returned
    ...
    console.error('OpenAI generation error, falling back to heuristic:', error.message);
    return {
      provider: 'heuristic',
      insights: [...analysis.insights, ...analysis.recommendations].slice(0, 8)
    };
